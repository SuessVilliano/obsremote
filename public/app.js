...
const STORAGE = { pin: 'obsremote-pin', target: 'obsremote-target', remember: 'obsremote-remember' };
...
function send(payload) {
  if (!socket || socket.readyState !== WebSocket.OPEN) { toast('Remote is not connected'); return; }
...
function connect({ pin = '', targetId, reconnecting = false } = {}) {
  ...
  socket.addEventListener('open', () => { reconnectAttempts = 0; els.loginCard.classList.add('hidden'); els.dashboard.classList.remove('hidden'); rememberConnection(pin, target); });
  socket.addEventListener('message', (event) => {
    ...
    if (message.type === 'event') {
      if (message.event === 'scene') { state.currentScene = message.sceneName; renderScenes(); }
...
document.querySelectorAll('.smart-action').forEach((button) => button.addEventListener('click', () => send({ action: 'smart-action', preset: button.dataset.preset })));
document.querySelectorAll('.background-card').forEach((button) => button.addEventListener('click', () => send({ action: 'apply-background', page: button.dataset.page })));
document.querySelectorAll('.sfx-card').forEach((button) => button.addEventListener('click', () => send({ action: 'play-sfx', sound: button.dataset.sound })));
