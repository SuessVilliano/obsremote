...
test('RTMP validator accepts only RTMP schemes with hosts and no embedded credentials',()=>{assert.equal(isRtmpUrl('rtmp://example.com/live'),true);assert.equal(isRtmpUrl('rtmps://example.com/app'),true);assert.equal(isRtmpUrl('https://example.com'),false);assert.equal(isRtmpUrl('rtmp:///missing'),false);assert.equal(isRtmpUrl('rtmps://user:secret@example.com/live'),false)});
