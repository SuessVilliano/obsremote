...
test('playlist normalization never accepts phantom tracks when library is empty',()=>{const p=normalizePlaylist({trackIds:['ghost']},[]);assert.deepEqual(p.trackIds,[])});
...
test('advanceState re-derives index from current track after playlist edits',()=>{const s={...DEFAULT_MUSIC_STATE,trackId:'b',index:0,playlistId:'p',loop:true,playing:true};const next=advanceState(s,{id:'p',trackIds:['a','b','c'],loop:true},1);assert.equal(next.trackId,'c');assert.equal(next.index,2)});
