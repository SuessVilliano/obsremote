...
export class ObsTargetManager extends EventEmitter{
  ...
  async #connect(){clearTimeout(this.reconnectTimer);this.#setState('connecting');const obs=new OBSWebSocket();this.obs=obs;this.#attach(obs);try{await obs.connect(this.target.url,this.target.password||'',{eventSubscriptions:EventSubscription.All});this.reconnectAttempt=0;this.lastError='';this.#setState('connected');return obs}catch(error){this.lastError=error?.message||String(error);this.#setState('error');try{await obs.disconnect()}catch{}if(this.obs===obs)this.obs=null;this.#scheduleReconnect();throw error}}
  #attach(obs){const relay=(name)=>(payload)=>this.emit('obs-event',{name,payload});for(const name of ['CurrentProgramSceneChanged','InputMuteStateChanged','InputVolumeChanged','SceneItemEnableStateChanged','StreamStateChanged','RecordStateChanged','SceneListChanged','InputCreated','InputRemoved','InputNameChanged'])obs.on(name,relay(name));obs.on('ConnectionClosed',()=>{if(this.obs!==obs)return;this.obs=null;this.lastError='OBS connection closed';this.#setState('offline');this.#scheduleReconnect()})}
